...
top_10_betweenness = sorted(g_2024.betweenness_centrality.items(), key=lambda x: x[1], reverse=True)[:10]
...
top_10_load = sorted(g_2024.load_centrality.items(), key=lambda x: x[1], reverse=True)[:10]
...
for as_id, bc in g_2024.betweenness_centrality.items():
    country = iso_a2_to_iso_a3(g_2024.as_to_country.get(as_id, 'Unknown'))
    country_betweenness[country] = country_betweenness.get(country, 0) + bc  * 100 + 1e-6
    country_load[country] = country_load.get(country, 0) + g_2024.load_centrality.get(as_id, 0) * 100 + 1e-6
...
for country in countries:
    plt.plot(range(1, len(g_2024.crp[country]) + 1), g_2024.crp[country], label=country)
...
plt.figure(figsize=(10, 8))
for country in countries:
    plt.plot(range(1, len(g_2024.grp[country]) + 1), g_2024.grp[country], label=country)
